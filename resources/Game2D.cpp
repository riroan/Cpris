...
Game2D &Game2D::init(const std::string &_title, const int &_width, const int &_height, const bool &use_full_screen, const int &display_ix)
{
    ...
    std::cout << "Display width = " << width << " height = " << height << " Aspect ratio is " << aspect_ratio << std::endl;
...
{
    if (key_status.count(key) <= 0)
        key_status[key] = false; // register key to map
...
{
    if (mbtn_status.count(mbtn) <= 0)
        mbtn_status[mbtn] = false; // register key to map
